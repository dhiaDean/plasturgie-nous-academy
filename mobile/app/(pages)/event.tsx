...
const formatFullDateTime = (dateString: string | undefined) => {
    ...
    return date.toLocaleDateString('fr-FR', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit', hour12: false });
};
...
export default function EventsScreen() {
    ...
    const fetchData = useCallback(async (isRefresh: boolean = false) => {
        ...
        try {
            ...
            const message = err?.response?.data?.message || err?.message || "Impossible de charger les événements.";
            setError(message);
    ...
    const handleApplyDateFilter = () => {
        if (tempStartDate && tempEndDate && tempStartDate > tempEndDate) {
            Alert.alert("Plage invalide", "La date de début ne peut pas être postérieure à la date de fin.");
            return;
        }
    ...
    const handleRegister = (eventId: number | undefined) => {
        ...
        Alert.alert("Inscription", `Simulation d'inscription pour l'événement ID: ${eventId}`);
    ...
    const dateFilterButtonText = () => {
        ...
        if (startDate) return `Date: À partir du ${formatDateDisplay(startDate)}`;
        if (endDate) return `Date: Jusqu'au ${formatDateDisplay(endDate)}`;
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            <Text style={styles.pageTitle}>Événements à venir</Text>
            ...
            >
                {error && !isLoading && (
                    <View style={styles.errorBox}>
                        <Text style={styles.errorTextMsg}>Erreur : {error}</Text>
                ...
                {!isLoading && !error && filteredEvents.length === 0 && (
                    <View style={styles.emptyStateContainer}>
                         ...
                         <Text style={styles.emptyStateText}>
                             Aucun événement ne correspond à vos filtres actuels. Essayez d'ajuster votre recherche ou d'effacer les filtres.
                         </Text>
                         <TouchableOpacity style={styles.clearButtonLarge} onPress={handleClearFilters}>
                             <Text style={styles.clearButtonText}>Voir tous les événements</Text>
                         </TouchableOpacity>
                    </View>
                )}
                                    ...
                                    {(event.maxParticipants !== undefined) && (
                                        <View style={styles.eventDetailRow}>
                                            ...
                                            <Text style={styles.eventDetailText}>
                                                ...
                                                {event.maxParticipants > 0 ? `${event.maxParticipants} Participants` : 'Illimité'}
                                            </Text>
                                        </View>
                                    )}
                                </View>
                                <TouchableOpacity style={styles.registerButton} onPress={() => handleRegister(event.id ?? event.eventId)}>
                                    <Text style={styles.registerButtonText}>S'inscrire / Voir détails</Text>
                                </TouchableOpacity>
                            </View>
                        ))}
                    </View>
                    ...
                    <TouchableOpacity style={styles.modalContent} activeOpacity={1} onPress={() => {}}>
                        ...
                        <View style={styles.datePickerRow}>
                            ...
                            <TouchableOpacity style={styles.dateDisplayButton} onPress={() => setShowPicker('start')}>
                                <Text style={styles.dateDisplayText}>{tempStartDate ? formatDateDisplay(tempStartDate) : 'Sélectionner début'}</Text>
                            </TouchableOpacity>
                        </View>
                        <View style={styles.datePickerRow}>
                            <Text style={styles.datePickerLabel}>Au :</Text>
                            <TouchableOpacity style={styles.dateDisplayButton} onPress={() => setShowPicker('end')}>
                                <Text style={styles.dateDisplayText}>{tempEndDate ? formatDateDisplay(tempEndDate) : 'Sélectionner fin'}</Text>
                            </TouchableOpacity>
                        </View>
                        {showPicker && (
                            <DateTimePicker
                        ...
                        <View style={styles.modalFooter}>
                             <Button title="Effacer dates" onPress={() => {setTempStartDate(null); setTempEndDate(null);}} color={Platform.OS === 'ios' ? "#007AFF" : undefined}/>
                             <View style={{width: 10}}/>
                             <Button title="Annuler" onPress={() => setModalVisible(false)} color={Platform.OS === 'ios' ? "#FF3B30" : undefined}/>
                             <View style={{width: 10}}/>
                             <Button title="Appliquer" onPress={handleApplyDateFilter} />
                        </View>
                    </TouchableOpacity>
                </TouchableOpacity>
            </Modal>
